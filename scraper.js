...
rl.question(`What are you Looking for (Write in plural)?\n`, (srchStr) => {
    return (async() => {
        try {
            ...
            let bangladesh = await fs.promises.readFile("location.json");
            bangladesh = JSON.parse(bangladesh);
            for (let i = 0; i < bangladesh.length; i++) {
                let regions = Object.keys(bangladesh[i]);
                for (let region = 0; region < regions.length; region++) {
                    let provinces = Object.keys(bangladesh[i][regions[region]]);
                    for (let province = 0; province < provinces.length; province++) {
                        //all the code will sit inside the body of this loop
                        let towns = bangladesh[i][regions[region]][provinces[province]];
                        for (let town = 0; town < towns.length; town++) {
                            let urlStr = `${srchStr}+in+${towns[town]},${provinces[province]} ${regions[region]}`;
                            ...
                            for (let i = 0; i < elementNames.length; i++) {
                                ...
                                let newUrl = `https://www.google.com/maps/search/${elementNames[i]} in ${towns[town]},${provinces[province]} ${regions[region]}/`;
